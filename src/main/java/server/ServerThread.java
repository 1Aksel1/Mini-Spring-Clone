...
public class ServerThread implements Runnable{
    ...
    public void run(){
        try {
            ...
            Response response = WebFramework.handleRequest(request.getMethod() + request.getLocation(), request.getParameters());
    ...
    private Request generateRequest() throws IOException {
        ...
        if(actionRow[1].contains("?")){
            route = actionRow[1].split("\\?")[0];
        }
